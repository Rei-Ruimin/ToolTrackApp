...
class TaskDB {
  ...
  Future updateTask(Tasks task, {List<int>? labelIDs}) async {
    ...
    await db.transaction((Transaction txn) async {
      ...
      // Rei
          '${Tasks.tblTask}(${Tasks.dbId},${Tasks.dbTitle},${Tasks.dbProjectID},${Tasks.dbComment},${Tasks.dbDueDate},${Tasks.dbFromTime},${Tasks.dbToTime},${Tasks.dbUseTool},${Tasks.dbPriority},${Tasks.dbStatus})'
          ' VALUES(${task.id}, "${task.title}", ${task.projectId},"${task.comment}", ${task.dueDate},${task.fromTime},${task.toTime},${task.useTool},${task.priority.index},${task.tasksStatus!.index})');
